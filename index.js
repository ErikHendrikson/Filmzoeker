...
// Filtered array for 2014 and newer movies
let movies2014 = movies.filter(filterFunction);
function filterFunction (movie){
    return movie.Year >= 2014;
};
document.getElementById("2014-en-nieuwer").addEventListener("change", function (){
    removeAllChildren (gridContainer);
    deploy (movies2014);
    });

// Filtered array for Avengers, X-Men, Princess and Batman
document.querySelectorAll(".filterclass").forEach(filterclass => filterclass.addEventListener("change", function (){  
    removeAllChildren (gridContainer);
    let filterId = this.id;
    let newList = movies.filter(filterFunction2);
    function filterFunction2 (movie){
        return movie.Title.includes(filterId);}
    deploy(newList);
}));
...
// Deploy function, to append children elements to the gridcontainer when called
function deploy (films){
for (let i=0; i < films.length; i++){
const el = document.createElement("a");
const elImg = document.createElement("img");
gridContainer.appendChild(el);
el.appendChild(elImg);
elImg.setAttribute('src', films[i].Poster);
el.setAttribute('href', `https://www.imdb.com/title/${films[i].imdbID}/`);
}};

// Removes all children elements
function removeAllChildren (gridContainer){
    while (gridContainer.firstChild) {
        gridContainer.removeChild(gridContainer.firstChild);
    }
}

// Deploy movieslist on pageload
deploy (movies);